...
impl BlinkManager {
    ...
    /// For non-smooth-blink: same as `visible()`.
    /// For smooth-blink: also true while the fade is in progress (opacity > 0),
    /// so the cursor remains on-screen during the fade-out.
