        ...
        let mut cursor_layouts = self.editor.update(cx, |editor, cx| {
            ...
            for (player_color, selections) in selections {
                for selection in selections {
                    ...
                    let document_y: Pixels = (cursor_position.row().as_f64()
                        * ScrollPixelOffset::from(line_height))
                    .into();
                    ...
                    let (x, y, quad_corners) = if selection.is_newest {
                        if let Some(quad) = editor.quad_cursor_mut() {
                            ...
                            // Animate in document space so scrolling does not lag the cursor.
...
struct CursorAnimationState {
    ...
    /// Viewport offset of the document origin; subtracted from the quad cursor's
    /// document-space position to paint it in viewport space.
